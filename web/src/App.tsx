...
export function App({ onGoHome }: AppProps) {
  ...
  const handleGitMakeCommit = useCallback(
    async (message: string, amend: boolean) => {
      const rootID = currentRootIdRef.current;
      if (!rootID) return;
      await gitMakeCommit(rootID, message, amend);
      clearGitHistoryCache(rootID);
      await Promise.all([
        refreshGitStatus(rootID),
        refreshGitHistory(rootID, { force: true }),
      ]);
    },
  ...
  const handleGitCommitAndPush = useCallback(
    async (message: string) => {
      ...
      await gitMakeCommit(rootID, message, false);
      clearGitHistoryCache(rootID);
      try {
        await gitRunRemoteAction(rootID, "push");
      } catch (err) {
        await Promise.all([
          refreshGitStatus(rootID),
          refreshGitHistory(rootID, { force: true }),
  ...
  const handleGitRemoteAction = useCallback(
    async (action: GitRemoteAction) => {
      ...
      try {
        const next = await gitRunRemoteAction(rootID, action);
        clearGitHistoryCache(rootID);
        if (currentRootIdRef.current === rootID) {
      ...
      <AppShell
        ...
        rightSidebar={
          <RightSidebar
            ...
            onGitCommit={handleGitMakeCommit}
            ...
            onGitRemoteAction={handleGitRemoteAction}
            onToggleGitHistory={() => setShowGitHistory((value) => !value)}
            onToggleGitHistoryCommit={(hash) => {
              const root = currentRootIdRef.current;
              if (!root) {
                return;
              }
            ...
            onSelectGitHistoryFile={(commit, item) => {
              const root = currentRootIdRef.current;
              if (!root) {
                return;
              }
        ...
        footer={
          <ActionBar
            ...
            roots={managedRootIds.map((id) => ({
              id,
              display_name: managedRootByIdRef.current[id]?.display_name,
            ...
            onCheckoutBranch={async (branch) => {
              if (!currentRootId) return;
              await switchGitBranch(currentRootId, branch);
            }}
            onSessionClick={() => {
              const rootID = currentRootIdRef.current;
