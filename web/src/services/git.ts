...
export type GitRemoteAction = "pull" | "push" | "fetch";
...
export async function gitMakeCommit(rootId: string, message: string, amend = false): Promise<{ branch: string; message: string }> {
  const payload = await protectedJSON<any>(appURL("/api/git/commit/make"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ root: rootId, message, amend }),
  });
  return {
    branch: typeof payload?.result?.branch === "string" ? payload.result.branch : "",
    message: typeof payload?.result?.message === "string" ? payload.result.message : "",
  };
}

export async function gitStageFile(rootId: string, path: string): Promise<GitStatusPayload> {
...
export async function gitStageAll(rootId: string): Promise<GitStatusPayload> {
  const payload = await protectedJSON<any>(appURL("/api/git/stage"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ root: rootId, all: true }),
  });
  return normalizeStatusPayload(payload);
}

export async function gitUnstageFile(rootId: string, path: string): Promise<GitStatusPayload> {
...
function normalizeStatusPayload(payload: any): GitStatusPayload {
  return {
    available: payload?.available === true,
    branch: typeof payload?.branch === "string" ? payload.branch : undefined,
    dirty_count: Number(payload?.dirty_count) || 0,
...
export async function gitRunRemoteAction(
  rootId: string,
  action: GitRemoteAction,
): Promise<{ action: GitRemoteAction; output: string; status: GitStatusPayload }> {
  const payload = await protectedJSON<any>(appURL("/api/git/remote"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ root: rootId, action }),
  });
  return {
    action,
    output: typeof payload?.output === "string" ? payload.output : "",
    status: normalizeStatusPayload(payload?.status || payload),
  };
}
