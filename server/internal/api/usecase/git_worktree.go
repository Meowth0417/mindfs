...
type GitRemoteActionInput struct {
	RootID string
	Action string
}

type GitRemoteActionOutput struct {
...
func (s *Service) GitRemoteAction(ctx context.Context, in GitRemoteActionInput) (GitRemoteActionOutput, error) {
	if err := s.ensureRegistry(); err != nil {
		return GitRemoteActionOutput{}, err
	}
	root, err := s.Registry.GetRoot(in.RootID)
	if err != nil {
		return GitRemoteActionOutput{}, err
	...
	if action == "" {
		return GitRemoteActionOutput{}, errors.New("action required")
	}
	output, err := gitview.RunRemoteAction(ctx, root.RootPath, action)
	if err != nil {
		return GitRemoteActionOutput{}, err
	}
	status, err := gitview.InspectStatus(ctx, root.RootPath)
