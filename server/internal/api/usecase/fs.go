...
type GitMakeCommitInput struct {
	RootID  string
	Message string
	Amend   bool
}

type GitMakeCommitOutput struct {
	Result gitview.CommitResult
	Status gitview.StatusResult
}
...
func (s *Service) GitMakeCommit(ctx context.Context, in GitMakeCommitInput) (GitMakeCommitOutput, error) {
	if err := s.ensureRegistry(); err != nil {
		return GitMakeCommitOutput{}, err
	}
	root, err := s.Registry.GetRoot(in.RootID)
	if err != nil {
		return GitMakeCommitOutput{}, err
	}
	result, err := gitview.DoCommit(ctx, root.RootPath, in.Message, in.Amend)
	if err != nil {
		return GitMakeCommitOutput{}, err
	}
	status, err := gitview.InspectStatus(ctx, root.RootPath)
	if err != nil {
		status = gitview.StatusResult{Available: true}
	}
	return GitMakeCommitOutput{Result: result, Status: status}, nil
}

func (s *Service) GitStage(ctx context.Context, in GitStageInput) (gitview.StatusResult, error) {
	if err := s.ensureRegistry(); err != nil {
		return gitview.StatusResult{}, err
	}
	root, err := s.Registry.GetRoot(in.RootID)
	if err != nil {
		return gitview.StatusResult{}, err
	...
	if in.All {
		err = gitview.StageAll(ctx, root.RootPath)
	} else {
		err = gitview.StageFile(ctx, root.RootPath, in.Path)
	}
	if err != nil {
		return gitview.StatusResult{}, err
	}
	return gitview.InspectStatus(ctx, root.RootPath)
}

func (s *Service) GitUnstage(ctx context.Context, in GitUnstageInput) (gitview.StatusResult, error) {
	if err := s.ensureRegistry(); err != nil {
		return gitview.StatusResult{}, err
	}
	root, err := s.Registry.GetRoot(in.RootID)
	...
	if err := gitview.UnstageFile(ctx, root.RootPath, in.Path); err != nil {
		return gitview.StatusResult{}, err
	}
	return gitview.InspectStatus(ctx, root.RootPath)
}
