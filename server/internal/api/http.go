...
func (h *HTTPHandler) Routes() http.Handler {
	...
	r.Post("/api/git/remote", h.protectedEndpoint(h.handleGitRemoteAction))
	r.Post("/api/git/commit/make", h.protectedEndpoint(h.handleGitMakeCommit))
...
func pathForStaticAsset(requestPath string) string {
	if relayPrefix := relayNodePrefix(requestPath); relayPrefix != "" {
		requestPath = strings.TrimPrefix(requestPath, relayPrefix)
		if requestPath == "" {
			requestPath = "/"
		}
	}
	if strings.HasPrefix(requestPath, "/mindfs-assets/") {
		requestPath = "/assets/" + strings.TrimPrefix(requestPath, "/mindfs-assets/")
	}
	cleaned := filepath.Clean("/" + requestPath)
	if cleaned == "/" {
		return ""
	}
	trimmed := strings.TrimLeft(filepath.ToSlash(cleaned), "/")
	return trimmed
}

func relayNodePrefix(requestPath string) string {
	if !strings.HasPrefix(requestPath, "/n/") {
		return ""
	}
	rest := strings.TrimPrefix(requestPath, "/n/")
	if rest == "" {
		return ""
	}
...
func (h *HTTPHandler) handleVSCodeOpen(w http.ResponseWriter, r *http.Request) {
	if !isLoopbackRequest(r) {
		respondError(w, http.StatusForbidden, errInvalidRequest("vscode launch is only available on local connections"))
		return
	}
	var req struct {
	...
	uc := h.service()
	out, err := uc.LaunchVSCode(r.Context(), usecase.LaunchVSCodeInput{
		RootID: req.RootID,
		Path:   req.Path,
		Line:   req.Line,
		Column: req.Column,
...
func (h *HTTPHandler) handleGitStage(w http.ResponseWriter, r *http.Request) {
	...
	if !req.All && strings.TrimSpace(req.Path) == "" {
		respondError(w, http.StatusBadRequest, errInvalidRequest("path required"))
		return
	}
	uc := h.service()
	status, err := uc.GitStage(r.Context(), usecase.GitStageInput{
		RootID: req.RootID,
		Path:   strings.TrimSpace(req.Path),
		All:    req.All,
	})
	if err != nil {
		respondJSON(w, http.StatusConflict, map[string]any{"error": "git_stage_failed", "message": err.Error()})
		return
	}
...
func (h *HTTPHandler) handleGitUnstage(w http.ResponseWriter, r *http.Request) {
	...
	if req.RootID == "" || req.Path == "" {
		respondError(w, http.StatusBadRequest, errInvalidRequest("root and path required"))
		return
	}
	uc := h.service()
	status, err := uc.GitUnstage(r.Context(), usecase.GitUnstageInput{
		RootID: req.RootID,
		Path:   req.Path,
	})
	if err != nil {
		respondJSON(w, http.StatusConflict, map[string]any{"error": "git_unstage_failed", "message": err.Error()})
		return
	}
	respondJSON(w, http.StatusOK, status)
}

func (h *HTTPHandler) handleGitMakeCommit(w http.ResponseWriter, r *http.Request) {
	var req struct {
		RootID  string `json:"root"`
	...
	uc := h.service()
	out, err := uc.GitMakeCommit(r.Context(), usecase.GitMakeCommitInput{
		RootID:  req.RootID,
		Message: req.Message,
		Amend:   req.Amend,
	})
...
func (h *HTTPHandler) handleGitRemoteAction(w http.ResponseWriter, r *http.Request) {
	var req struct {
		RootID string `json:"root"`
		Action string `json:"action"`
	}
	...
	if req.Action == "" {
		respondError(w, http.StatusBadRequest, errInvalidRequest("action required"))
		return
	}
	uc := h.service()
	out, err := uc.GitRemoteAction(r.Context(), usecase.GitRemoteActionInput{
		RootID: req.RootID,
		Action: req.Action,
	})
	if err != nil {
