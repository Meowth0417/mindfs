...
type StatusItem struct {
	...
	Area      string `json:"area,omitempty"` // "staged" | "unstaged"
}
...
func RunRemoteAction(ctx context.Context, rootPath, action string) (string, error) {
	repo, err := loadRepoContext(ctx, rootPath)
	if err != nil {
		return "", err
	}
	...
	case "pull":
		return runGit(ctx, repo.repoRoot, "pull", "--ff-only")
	case "push":
		return runGit(ctx, repo.repoRoot, "push")
	case "fetch":
		return runGit(ctx, repo.repoRoot, "fetch", "--prune")
	default:
		return "", errors.New("unsupported git remote action")
	}
}

func DoCommit(ctx context.Context, rootPath, message string, amend bool) (CommitResult, error) {
	repo, err := loadRepoContext(ctx, rootPath)
	if err != nil {
	...
	if !amend && message == "" {
		return CommitResult{}, errors.New("commit message required")
	}
	args := []string{"commit"}
	if amend {
	...
	return CommitResult{Branch: repo.branch, Message: message}, nil
}
...
func (r repoContext) statusItems(ctx context.Context) ([]StatusItem, error) {
	...
	// Separate into staged (index) and unstaged (worktree) entries.
	var stagedRaw, unstagedRaw []porcelainItem
	for _, item := range rawItems {
		if item.X != ' ' && item.X != '?' {
			stagedRaw = append(stagedRaw, item)
		}
		if item.Y != ' ' {
			unstagedRaw = append(unstagedRaw, item)
		}
	}

	// Fetch numstats: --cached for staged, plain for unstaged.
	stagedStats := make(map[string][2]int)
	if len(stagedRaw) > 0 {
		out, err := r.batchNumstatHelper(ctx, true, stagedRaw)
		if err == nil {
			parseBatchNumstat(out, stagedStats)
		}
	}
	unstagedStats := make(map[string][2]int)
	if len(unstagedRaw) > 0 {
		out, err := r.batchNumstatHelper(ctx, false, unstagedRaw)
		if err == nil {
			parseBatchNumstat(out, unstagedStats)
		}
	}

	items := make([]StatusItem, 0, len(stagedRaw)+len(unstagedRaw))

	for _, item := range stagedRaw {
		path := r.fromRepoPath(item.Path)
		if path == "" || shouldIgnoreStatusPath(path) {
			continue
		}
		path = strings.TrimSuffix(path, "/")
		oldPath := r.fromRepoPath(item.OldPath)
		repoPath := r.toRepoPath(path)
		stat := stagedStats[repoPath]
		items = append(items, StatusItem{
			Path:      path,
			OldPath:   oldPath,
			Status:    normalizeIndexStatus(item.X),
			Additions: stat[0],
			Deletions: stat[1],
			Area:      "staged",
		})
	...
	for _, item := range unstagedRaw {
		...
		if item.X == '?' && item.Y == '?' {
			...
			if info, err := os.Stat(target); err == nil && info.IsDir() {
				...
				lines, _ := countFileLines(target)
				additions = lines
			}
		} else {
			repoPath := r.toRepoPath(path)
			stat := unstagedStats[repoPath]
			additions = stat[0]
			deletions = stat[1]
		}
		items = append(items, StatusItem{
			Path:      path,
			OldPath:   oldPath,
			Status:    normalizeWorktreeStatus(item.X, item.Y),
			Additions: additions,
			Deletions: deletions,
			IsDir:     isDir,
			Area:      "unstaged",
...
func parsePorcelainV1Z(data []byte) ([]porcelainItem, error) {
	...
	for index < len(data) {
		...
		index += next + 1
		item := porcelainItem{Path: path, Status: normalizeStatus(x, y), X: x, Y: y}
		if x == 'R' || y == 'R' || x == 'C' || y == 'C' {
...
func normalizeIndexStatus(x byte) string {
	switch x {
	case 'A':
		return "A"
	case 'D':
...
func normalizeWorktreeStatus(x, y byte) string {
	if x == '?' && y == '?' {
		return "??"
	}
	switch y {
...
func StageFile(ctx context.Context, rootPath, path string) error {
	repo, err := loadRepoContext(ctx, rootPath)
	if err != nil {
		return err
	}
	_, err = runGit(ctx, repo.repoRoot, "add", "--", repo.toRepoPath(path))
	return err
}

func UnstageFile(ctx context.Context, rootPath, path string) error {
	repo, err := loadRepoContext(ctx, rootPath)
	if err != nil {
		return err
	}
	_, err = runGit(ctx, repo.repoRoot, "restore", "--staged", "--", repo.toRepoPath(path))
	return err
}

func StageAll(ctx context.Context, rootPath string) error {
	repo, err := loadRepoContext(ctx, rootPath)
	if err != nil {
		return err
	}
	...
	_, err = runGit(ctx, repo.repoRoot, "add", "--", scope)
	return err
}
